    ...
    public static class BestiaryReader
    {
        ...
        public static string ReadFormation(int formationIndex, MonsterParty party)
        {
            ...
            try
            {
                ...
                string members = names.Count == 0 ? T("Empty") : string.Join(", ", names);
                return string.Format(T("Formation {0}: {1}"), formationIndex + 1, members);
            }
            catch (Exception ex)
            {
                MelonLogger.Warning($"[Bestiary] Error reading formation: {ex.Message}");
                return string.Format(T("Formation {0}: {1}"), formationIndex + 1, T("Unknown"));
            }
        }
        ...
        private static string ResolveMonsterName(int monsterId)
        {
            try
            {
                ...
                string fallback = string.Format(T("Monster {0}"), monsterId);
                if (masterManager == null) return fallback;
                ...
                if (string.IsNullOrEmpty(mesId)) return fallback;

                return LocalizationHelper.GetGameMessage(mesId) ?? fallback;
            }
            catch
            {
